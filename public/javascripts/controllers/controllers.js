app.controller("eveController", ["$scope", "eveService", '$location', "$window", function($scope, eveService, $location, $window){
  ...
  var jitaMarket = {regionId: 10000002, stationId: 60003760};
  var amarrMarket = {regionId: 10000043, stationId: 60008494};
  var allMarkets = [];
  allMarkets.push(jitaMarket);
  allMarkets.push(amarrMarket);

  if($location.path() == '/home'){

    for (var iMarket = 0; iMarket < allMarkets.length; iMarket++) {
//      getMarketOrders(allMarkets[i], wrapArrForThisStation); 
      getMarketOrders(allMarkets[i]);
    }
...
function getMarketOrders(var marketInfo, var wrapArr) {

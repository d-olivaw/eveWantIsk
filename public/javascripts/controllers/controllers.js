app.controller("eveController", ["$scope", "eveService", '$location', "$window", function($scope, eveService, $location, $window){
  ...
  function getMarketOrders(regionId, stationId, regName){
    for (var i = 0; i < $scope.view.itemArr.length; i++) {
      ...
      eveService.grabSell(regionId, itemIdinArr).then(function(data){
        var innest = data['data']['items'];
        var accumVolSell = 0;
        var newArr = [];
        for (var i = 0; i < innest.length; i++) {
          if(innest[i]['location']['id'] == stationId){
        ...
        eveService.grabBuy(regionId, itemIdinArr).then(function(data){
          var getinnest = data['data']['items']
          var newArr = [];
          for (var i = 0; i < getinnest.length; i++) {
            if(getinnest[i]['location']['id'] == stationId){
        ...
        eveService.grab7Day(regionId, itemIdinArr).then(function(data){
          var getinnest = data['data']['items']
          var average = 0;
          var nDay = 7;
